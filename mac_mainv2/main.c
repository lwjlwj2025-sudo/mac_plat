...
// 在 main.c 中定义
void perf_report_callback(void *arg) {
    static float sim_clock = 0;
    sim_clock += 1.0f; // 每次被调用代表经过了1秒
    Print_Performance_Report(sim_clock);
}
int main()
{
    // /* 系统初始化 */
    ...
    // // pcieInit();
    stimer_queue_init();
    stimer_queue_add_us("mac_perf", perf_report_callback, NULL, 0, 1000000);
    LOG("Sat_mac_u System initialization is complete");
    printCurrentTime();
    // //*********************         开数据接收线程          *****************************//
    // 创建来自Sat RRC的Package接收子线程
    App_queue_init(&Queue_RACH_Req); // [NEW] 初始化 RACH 队列
    pthread_mutex_init(&Global_MAC_Lock, NULL); // [NEW] 初始化锁

    // 2. 启动 RACH 线程
    pthread_t tid_rach;
    pthread_create(&tid_rach, NULL, Thread_RACH_Process, NULL);
    ...
    // [NEW] 初始化干扰矩阵 (必须在 Topology 之后)
    Init_Interference_Matrix(); 
    
    Generate_Users_And_Match();
