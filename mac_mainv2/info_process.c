...
// #include    "App_queue.h"
#include "App_udp.h"
u8 QoS_Mapping(u8 service_type) {
    switch (service_type) {
        case SERVICE_VOICE: return 1; // High
        case SERVICE_VIDEO: return 2; // Mid
        case SERVICE_WEB:   return 2;
        case SERVICE_BACKGROUND: return 3; // Low
        default: return 3;
    }
}
void RAMSG_process(msg_all *msg) // handle rrc info
{
    g_MacStats.rach_attempts++; // 统计尝试次数
    ...
    u8 service_type = RAMSG_frame->Service_Type; // [NEW]
    ...
    LOG("[RACH] Recv Msg A from UE %d (Service: %d) @ Beam %d\n", 
        get_ue_rnti, service_type, wave_locall_index);
    ...
    else
    {
        ...
        for (index; index < PER_WAVE_MAX_NUM; index++)
        {
            if (wave_ueser_mapPtrArray[wave_locall_index][index].State == 0)
            {
                ...
                // [完善点]：计算并存入位置因子 L_j
                wave_ueser_mapPtrArray[wave_locall_index][index].Location_Factor = 
                calc_location_factor(wave_locall_index, get_ue_rnti);

                LOG("[User-Init] UE %d Location_Factor: %.2f\n", 
                get_ue_rnti, wave_ueser_mapPtrArray[wave_locall_index][index].Location_Factor);
                // 下边两个值应该由用户的REQ传上来，现在先随便给个值
                wave_ueser_mapPtrArray[wave_locall_index][index].UE_Level = QoS_Mapping(service_type);
                wave_ueser_mapPtrArray[wave_locall_index][index].Is_RealTime = 
                (service_type == SERVICE_VOICE || service_type == SERVICE_VIDEO) ? 1 : 0;
                wave_ueser_mapPtrArray[wave_locall_index][index].Head_Of_Line_Delay = 0;
                wave_ueser_mapPtrArray[wave_locall_index][index].Avg_Data_Rate = 1000; // 避免除零
        ...
        // [LOG] 打印接入详情
        LOG("[RACH] UE %d Request | Service: %d (%s) | Beam: %d", 
        get_ue_rnti, service_type, 
        (service_type <= 2 ? "RT" : "nRT"), wave_locall_index);
        if (allocated) {
        // 4. 触发排序 (新接入用户可能优先级很高)
            sortLevel_MLRA(wave_ueser_mapPtrArray, wave_locall_index);
            g_MacStats.rach_success++;
        // [LOG] 打印 QoS 映射结果
            LOG("       -> Success | Mapped Level: %d | Initial Priority: %.2f\n", 
            wave_ueser_mapPtrArray[wave_locall_index][index].UE_Level,
            wave_ueser_mapPtrArray[wave_locall_index][index].Dynamic_Priority);
            // 5. 发送 MSG B (RARSP)
            // (此处复用原有 Package_RARSP 代码，省略细节)
            u8 RARSP_buff[128] = {0};
            ...
            access_ue_count++;
            LOG("[RACH] Sent Msg B (RARSP) to UE %d. Access Complete.\n", get_ue_rnti);
        } else {
            g_MacStats.rach_collisions++;
            LOG("[RACH] Access Failed: Beam Full.\n");
        }
        // 为当前虚波位上的用户按照用户优先级排序
...
// 计算位置因子 L_j = 10 * exp(-d^2 / beta^2)
double calc_location_factor(u16 wave_id, u32 rnti) {
    // 1. 获取用户在拓扑数组中的索引 (假设 RNTI 起始为 1000)
    int ue_topo_idx = rnti - 1000; 
    if (ue_topo_idx < 0 || ue_topo_idx >= UE_TOTALNUM) {
        return 1.0; // 防御性编程：返回默认最小值
    }

    // 2. 从拓扑模块获取用户到波束中心的距离 d
    double d = g_User_Topology[ue_topo_idx].Distance;
    
    // 3. 套用论文公式 (beta^2 定义在 CONSTANTS.h 中，例如 25.0)
    // L_j = 10 * exp(-d^2 / beta^2)
    double l_j = 10.0 * exp(-(d * d) / MLRA_BETA_SQ);
    
    return l_j;
}
double get_Location_Factor(u32 mcs) {
    // 简单映射：MCS 0-29 -> Factor 1.0 - 10.0
    return 1.0 + (double)mcs * 0.3; 
}
// 对应 MATLAB: R(i) = (1 - 1/tc)*R(i) + (1/tc)*r(i)
void update_average_rate(UserInfo_Block *ue, u32 current_tx_bytes) {
    double alpha = 1.0 / MLRA_TC;
    // 如果本时隙发送了数据，r(i) = current_tx_bytes，否则 r(i) = 0
    double instant_rate = (double)current_tx_bytes; 
    
    ue->Avg_Data_Rate = (1.0 - alpha) * ue->Avg_Data_Rate + alpha * instant_rate;
    
    // 防止下溢导致除零错误
    if(ue->Avg_Data_Rate < 1.0) ue->Avg_Data_Rate = 1.0;
}

// 核心优先级公式 (Paper Eq 6 & 7)
void calculate_user_priority(UserInfo_Block *ue) {
    if (ue->State == 0) {
        ue->Dynamic_Priority = -100.0;
        return;
    }

    double r_j = (double)cal_RB_carry_capacity(ue->MCS); // 瞬时信道容量
    double R_j = ue->Avg_Data_Rate;                      // 历史平均速率
    double q_j = (double)ue->User_DataQueue_ptr->size;   // 队列长度
    double W_j = (double)ue->Head_Of_Line_Delay;         // 时延
    double kappa_j = (double)ue->MCS;                    // 信道质量指标
    double L_j = ue->Location_Factor;                    // 位置因子

    double k_j = 0.0;

    // 论文 Eq(6): RT Service
    if (ue->Is_RealTime) {
        // k_j = w * (a_j * (r_j/R_j) * W_j * q_j) + (1-w) * (L_j * log(1+kappa_j))
        // 注意：这里为了数值稳定性，对 q_j 做了对数处理或归一化是常见的工程手段
        // 但为了严格遵循论文，我们按公式写：
        double term1 = MLRA_ALPHA_RT * (r_j / R_j) * W_j * q_j; 
        double term2 = L_j * log2(1.0 + kappa_j);
        k_j = MLRA_OMEGA * term1 + (1.0 - MLRA_OMEGA) * term2;
    } 
    // 论文 Eq(7): nRT Service
    else {
        // k_j = w * ((r_j/R_j) * q_j) + (1-w) * (L_j * log(1+kappa_j))
        double term1 = (r_j / R_j) * q_j;
        double term2 = L_j * log2(1.0 + kappa_j);
        k_j = MLRA_OMEGA * term1 + (1.0 - MLRA_OMEGA) * term2;
    }

    ue->Dynamic_Priority = k_j;
}

// 排序函数 (冒泡降序)
void sortLevel_MLRA(UserInfo_Block UEinWaveTable[WAVE_NUM][PER_WAVE_MAX_NUM], u16 wave_locall_index) {
    // 1. 更新所有优先级
    for(int i=0; i<PER_WAVE_MAX_NUM; i++) {
        if(UEinWaveTable[wave_locall_index][i].State == 1) {
            calculate_user_priority(&UEinWaveTable[wave_locall_index][i]);
        }
    }
    // 2. 排序
    for(int i=0; i<PER_WAVE_MAX_NUM-1; i++) {
        for(int j=0; j<PER_WAVE_MAX_NUM-i-1; j++) {
            // 简单冒泡，优先级高的浮上来
            UserInfo_Block *u1 = &UEinWaveTable[wave_locall_index][j];
            UserInfo_Block *u2 = &UEinWaveTable[wave_locall_index][j+1];
            
            if (u2->State == 1 && (u1->State == 0 || u2->Dynamic_Priority > u1->Dynamic_Priority)) {
                UserInfo_Block temp = *u1;
                *u1 = *u2;
                *u2 = temp;
            }
        }
...
// 计算某个波束的潜在干扰值 (周围有多少个已激活/高负载波束)
// 这里的逻辑是：如果我激活了这个波束，会受到多少来自其他“计划激活波束”的干扰？
// 在单星调度中，我们模拟选择“本星”最适合的一个波束
double calculate_beam_interference(u16 candidate_wave_id) {
    double interference_score = 0.0;
    
    // 遍历所有波束，看哪些波束是“活跃”的
    // 由于我们是在做单个BBU的决策，我们假设其他高负载波束也可能被调度
    // 这里使用静态干扰矩阵 * 目标波束的负载 来估算
    for (int i = 0; i < WAVE_NUM; i++) {
        if (i == candidate_wave_id) continue;
        
        // 如果波束 i 和 候选波束 有干扰 (距离 < 2R)
        if (g_Interference_Matrix[candidate_wave_id][i] == 1) {
            // 检查波束 i 的负载
            u32 load = 0;
            for(int u=0; u<PER_WAVE_MAX_NUM; u++) {
                if(wave_ueser_mapPtrArray[i][u].State == 1 && wave_ueser_mapPtrArray[i][u].User_DataQueue_ptr)
                    load += wave_ueser_mapPtrArray[i][u].User_DataQueue_ptr->size;
            }
            // 如果干扰源负载很高，它被调度的概率大，干扰风险就大
            if (load > 0) {
...
// 引入 frame_t 全局变量，用于获取当前时隙信息
extern frame_info *frame_t; 

// [修改版] 波束选择函数：增加详细 Log
void poll_wave_index_renew_MLRA() {
    double max_score = -99999.0;
    u16 best_wave = poll_wave_index; // 默认保留上一轮
    
    // 用于日志记录详细信息
    double best_demand = 0.0;
    double best_interf = 0.0;

    // 本 BBU 管理的波束范围
    u16 start_wave = BBU_index * WAVE_NUM_PER_BBU;
    u16 end_wave = start_wave + WAVE_NUM_PER_BBU;

    // 遍历候选波束
    for (u16 w = start_wave; w < end_wave; w++) {
        u16 local_w = w - start_wave; 
        
        // 1. 计算负载 (Demand)
        double demand = 0.0;
        for (int u = 0; u < PER_WAVE_MAX_NUM; u++) {
            UserInfo_Block *ue = &wave_ueser_mapPtrArray[local_w][u];
            if (ue->State == 1 && ue->User_DataQueue_ptr) {
                demand += (double)ue->User_DataQueue_ptr->size;
            }
        }
        ...
        // 2. 计算干扰 (Interference)
        double interference = calculate_beam_interference(w);

        // 3. 综合打分
        double score = demand - (interference * INTERF_PENALTY * 1000.0);

        if (score > max_score) {
            max_score = score;
            best_wave = local_w;
            best_demand = demand;
            best_interf = interference;
        }
    }

    // 更新决策 & 打印日志
    if (max_score > -90000.0) {
        poll_wave_index = best_wave;
        
        // [新增 LOG] 打印选中波束的详细信息
        // 格式：[时间] [BBU] 动作 | 选中波束(全局ID) | 得分细则
        LOG("[Slot %d.%d.%d] [BBU %d] BEAM_SWITCH | Select Local: %d (Global: %d) | Score: %.2f (Demand: %.0f, Interf: %.1f)\n",
            frame_t->frame_num, frame_t->sub_frame_num, frame_t->slot_num, // 时间戳
            BBU_index,
            best_wave, 
            start_wave + best_wave, // 全局波束ID
            max_score,
            best_demand,
            best_interf);
        ...
        // 轮询保底逻辑
        u16 old_wave = poll_wave_index;
        poll_wave_index = (poll_wave_index + 1) % WAVE_NUM_PER_BBU;
        
        // [新增 LOG] 打印轮询保底信息
        LOG("[Slot %d.%d.%d] [BBU %d] BEAM_POLL   | Idle Rotate: %d -> %d (Global: %d) | No High-Demand Beam\n",
            frame_t->frame_num, frame_t->sub_frame_num, frame_t->slot_num,
            BBU_index,
            old_wave, poll_wave_index,
            start_wave + poll_wave_index);
    }
}
