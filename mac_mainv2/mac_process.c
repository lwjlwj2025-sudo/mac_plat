...
// [NEW] 随机接入处理专用子线程
void *Thread_RACH_Process(void *arg) 
{
    LOG("[Thread] RACH Thread Started. Listening for Msg A...\n");
    
    u8 buffer[512]; // 接收缓冲区
    msg_all msg_wrapper;
    msg_head head_wrapper;
    
    while(1) 
    {
        // 1. 从队列中取出请求 (非阻塞或阻塞均可，这里用轮询+休眠模拟)
        if (!App_queue_empty(&Queue_RACH_Req)) 
        {
            // 从队列头部取出一个完整的 RAMSG 包
            // 注意：这里假设队列里存的是 payload，我们需要还原成 msg_all 结构传给 RAMSG_process
            // 或者队列里直接存的就是 type_RAMSG_s 结构体
            
            // 弹出数据 (假设一次弹出一个 type_RAMSG_s 大小)
            u32 pop_len = sizeof(type_RAMSG_s);
            if(App_queue_pop(&Queue_RACH_Req, buffer, pop_len, 0x01) == QUEUE_OK) // 0x01 是模拟从头部取
            {
                // 2. 封装成 RAMSG_process 需要的 msg_all 格式
                head_wrapper.TYPE = TYPE_RAMSG;
                head_wrapper.LEN = pop_len;
                head_wrapper.DATA[0] = buffer; // 指向刚才取出的数据
                // 注意：这里只是指针指向，因为 buffer 在本轮循环有效
                
                msg_wrapper.HEAD = &head_wrapper;

                // 3. 【关键】加锁调用 MAC 层处理函数
                // 因为 RAMSG_process 会修改全局用户表，主调度线程也会读写该表
                pthread_mutex_lock(&Global_MAC_Lock);
                
                RAMSG_process(&msg_wrapper);
